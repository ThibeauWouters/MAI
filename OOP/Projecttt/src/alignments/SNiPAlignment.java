 ...
 * and overrides some methods of StandardAlignment.
 * 
...
public class SNiPAlignment extends StandardAlignment {
	 ...
	 * 
	 * @param fasta A AlignmentsIO object, which holds the contents of a .fasta file.
	 * @param referenceID The ID of the reference genome.
	 */
	public SNiPAlignment(AlignmentsIO fasta, String referenceID) {
	...
	/**
	 * Constructor creating a SNiP alignment from a AlignmentsIO object, and use the first reference ID as the
	 * default value.
	 * 
	 * @param fasta A AlignmentsIO object, which holds the contents of a .fasta file.
	 */
	public SNiPAlignment(AlignmentsIO fasta) {		
	 ...
	 * 
	 * @param hmap A HashMap containing (ID, genome)-pairs to be stored as alignment.
	 ...
	 *
	 * @param sa The StandardAlignment from which we are going to copy the alignment.
	 * @param referenceID The ID of the reference genome used when constructing SNiP view.
	 */
	 ...
	 * Used in the constructors of SNiPAlignment to change from standard to SNiP view.
	 */
	...
	public ArrayList<String> searchGenomes(String subString){
		...
		for (String identifier : this.getIdentifiers()) {
			...
			// Reconstruct the standard view
			String sequence   = this.getOriginalGenome(difference);
			// If it contains substring, add to the list
	...
	public int getDifferenceScore() {
		...
		// Safer to create the dots ourselvse since one is allowed to delete genomes
		String comparisonString = "";
		for (int i = 0; i < this.getLengthGenomes(); i++) {
			comparisonString += '.';
		}
		// Get difference score using the standard way
	...
	public SNiPAlignment copy(){
		return new SNiPAlignment(this.copyAlignment());
	}
}
