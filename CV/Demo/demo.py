...
def sobel():
    ...
    # frame = cv2.merge(grad)

    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    grad = cv2.Sobel(gray, ddepth, 1, 0, ksize=3, borderType=cv2.BORDER_DEFAULT)
    # Convert the scale again
    grad = cv2.convertScaleAbs(grad)
    # Get the mask of ROI, where we have the Sobel gradients, and inverse mask
...
# sliders_HSV('frame.png')
# sliders_RGB('frame.png')
