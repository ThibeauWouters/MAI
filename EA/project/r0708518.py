...
class r0708518:
    ...
    def __init__(self, user_params_dict=None):
        ...
        # Note: "EX" and "GROUP" crossovers are not used anymore as they are inefficient compared to others!
        self.implemented_mutation_operators      = ["EM", "DM", "SIM", "ISM", "IVM", "SM", "SDM", "variable", "random"]
        self.implemented_recombination_operators = ["PMX", "SCX", "CX", "OX", "OX2", "AX", "variable", "random"]
        ...
        # --- DIVERSITY PROMOTION ---
        # Specify which metric we use and which are implemented (only Hamming was implemented in the end)
    ...
    def edge_crossover(self, parent1, parent2):
        """(EX) Performs the edge crossover operator. Not used, as it is very inefficient!"""
...
def make_fitness_plot(mean_fit_values, best_fit_values, filename, plot_mean=True, plot_name="plot_test_run",
    ...
    for k in heuristic_dict:
        if k + '.csv' in filename:
            tour_name = k
            heuristic_value = heuristic_dict[k]
            plt.axhline(heuristic_value, ls='--', color='black', alpha=0.7, label="Heuristic")
            break
...
def make_t50_histogram(filename="Runs/all_t50_data.csv", which="best"):
    ...
    print("Lowest  fitness value was %0.2f" % np.min(best))
    index = np.argmin(best)
    time_took = elapsed[index]
    print("Lowest  fitness run took  %0.2f seconds" % time_took)
    ...
    plt.figure()
    sns.histplot(values, bins=16, color='red', zorder=10, stat='density')
    sns.kdeplot(values, color='black', zorder=99, lw=3)
    ...
    plt.savefig('Plots/Project plots/histogram_t50_runs_' + which + '.png', bbox_inches='tight')
    plt.savefig('Plots/Project plots/histogram_t50_runs_' + which + '.pdf', bbox_inches='tight')
    plt.close()
