...
def shopSmart(orderList, fruitShops):
    ...
    # Get all prices for all stores
    shopPrices = []
    for shop in fruitShops:
        totalCost = 0
        for (fruit, numPounds) in orderList:
            if fruit not in shop.fruitPrices.keys():
                print("This is an error message")
                return None
            else:
                totalCost += numPounds * shop.fruitPrices[fruit]
        shopPrices.append(totalCost)

    # Now check which is best
    bestIndex = argmin(shopPrices)
    bestShop = fruitShops[bestIndex]
